<?php
// Standard pagination for standard posts
...
//Custom pagination for Fondsbørsmeddelelser
function fonds_pagination() {
		global $boers_query;
		...
		$paginate_links = paginate_links( array(
			'base' => str_replace( $big, '%#%', get_pagenum_link($big) ),
			'current' => max( 1, get_query_var('paged') ),
			'total' => $boers_query->max_num_pages,
			'mid_size' => 5,
			'prev_next' => True,
		    'prev_text' => __('&laquo;'),
		    'next_text' => __('&raquo;'),
...
//Custom pagination (Årsrapporter)
function rapport_pagination() {
		global $rapport_query;
		...
		$paginate_links = paginate_links( array(
			'base' => str_replace( $big, '%#%', get_pagenum_link($big) ),
			'current' => max( 1, get_query_var('paged') ),
			'total' => $rapport_query->max_num_pages,
			'mid_size' => 5,
			'prev_next' => True,
		    'prev_text' => __('&laquo;'),
		    'next_text' => __('&raquo;'),
