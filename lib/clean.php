...
if( ! function_exists( 'reverie_scripts_and_styles ' ) ) {
	function reverie_scripts_and_styles() {
	  if (!is_admin()) {
	  	// deregister wordpress jquery, adding CDN jquery. That way it stays cached and the site will load faster.
	    wp_deregister_script( 'jquery' );
	    wp_register_script( 'jquery', ( 'http://ajax.googleapis.com/ajax/libs/jquery/1.11.0/jquery.min.js' ), false, null, true );

	    // modernizr (without media query polyfill)
	    wp_register_script( 'modernizr', get_template_directory_uri() . '/js/modernizr.js', array(), '2.6.2', false );

	    // register Google font
	    wp_register_style('google-font', 'http://fonts.googleapis.com/css?family=Open+Sans:400,800,600');

		// register Google font
	    wp_register_style('google-font2', 'http://fonts.googleapis.com/css?family=Titillium+Web:400,600');	    
	    ...
	    // adding fastclick.js also in the footer
	    wp_register_script( 'fastclick-js', get_template_directory_uri() . '/js/fastclick.js', array( 'jquery' ), '', true );
